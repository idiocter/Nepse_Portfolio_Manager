...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white/95 backdrop-blur-md border border-zinc-100 rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl max-w-[280px] sm:max-w-xs">
        <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3 border-b border-zinc-50 pb-2">
          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: payload[0].fill }} />
          <p className="text-[10px] font-black text-zinc-900 uppercase tracking-widest leading-none mt-0.5 truncate">{data.name}</p>
        </div>

        <p className="text-lg sm:text-xl font-black text-zinc-900 tracking-tighter mb-3 sm:mb-4">
        ...
        <div className="space-y-1.5 sm:space-y-2 max-h-32 sm:max-h-40 overflow-y-auto custom-scrollbar pr-2">
          {data.stocks.map((stock, i) => (
            <div key={i} className="flex items-center justify-between gap-2 sm:gap-4 text-[9px] sm:text-[10px] font-bold">
              <span className="text-zinc-400 uppercase tracking-wider truncate">{stock.symbol}</span>
              <span className="text-zinc-900 tabular-nums flex-shrink-0">{stock.qty.toLocaleString()} Shares</span>
              <span className="text-zinc-500 tabular-nums flex-shrink-0">Rs. {stock.val.toLocaleString()}</span>
            </div>
          ))}
        </div>

        <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-zinc-50 flex items-center justify-between text-[8px] sm:text-[10px] font-bold text-zinc-300 uppercase tracking-widest">
  ...
  if (data.length === 0) {
    return (
      <div className="h-48 sm:h-64 flex flex-col items-center justify-center space-y-3 sm:space-y-4">
        <div className="p-3 sm:p-4 bg-zinc-50 rounded-xl sm:rounded-2xl border border-dashed border-zinc-200">
        ...
        <PieChart>
          ...
          <Legend
            ...
            formatter={(value, entry) => {
              ...
              return (
                <span className="text-[9px] sm:text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 mr-2 sm:mr-3">
